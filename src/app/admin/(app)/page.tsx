...
export default async function AdminDashboardPage() {
  ...
  const [upcoming, monthPayments, pendingCount, yearPayments] = await Promise.all([
    ...
    prisma.payment.findMany({
      where: { status: "PAYE", paidAt: { gte: new Date(currentYear, 0, 1), lt: new Date(currentYear + 1, 0, 1) } },
    }),
  ]);

  const monthTotalCents = monthPayments.reduce((sum, p) => sum + p.amountCents, 0);

  const chartTotals = MONTH_LABELS_SHORT.map((_, month) =>
    yearPayments
      .filter((p) => p.paidAt && p.paidAt.getMonth() === month)
  ...
  return (
    <div>
      ...
      <div className="mt-8 rounded-2xl border border-white/10 bg-[#16141c] p-6">
        ...
        <p className="mt-1 text-sm text-white/50">Encaissements par mois — {currentYear}</p>
        <div className="mt-8 overflow-x-auto">
          <div className="flex min-w-[560px] items-end justify-between gap-2 sm:min-w-0 sm:gap-4">
              ...
              return (
                <div key={i} className="flex flex-1 flex-col items-center">
                  <div className="flex h-48 w-full flex-col items-center justify-end">
                    <span className="mb-1 whitespace-nowrap text-[10px] font-semibold text-white/80 sm:text-sm">
                  ...
                  <span className="mt-2 text-[10px] uppercase tracking-wide text-white/40 sm:text-xs">
